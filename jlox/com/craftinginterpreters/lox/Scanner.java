...
//> slash
      case '/':
        if (match('/')) {
          ...
          while (peek() != '\n' && !isAtEnd()) advance();
        }else if(match('*')){ //we are at the start of a multi-line comment
            while(!isAtEnd()){
                if(peek() == '*' && peekNext() == '/'){ //skip contents until we hit the end of the block comment
                    advance();
                    advance();
                    break;
                }
                if(peek() == '\n'){ //multi line comment blocks have newlines, we need to update
                    line++;
                }
                advance();
            }
